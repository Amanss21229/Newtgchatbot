...
class Database:
    ...
    def start_chat_session(self, user1_id, user2_id):
        ...
        cursor = self.connection.cursor()
        placeholder = self._placeholder()
        # Update both users with chat partner
        cursor.execute(f'''
            UPDATE users SET chat_partner = {placeholder} WHERE user_id = {placeholder}
        ''', (user2_id, user1_id))
        cursor.execute(f'''
            UPDATE users SET chat_partner = {placeholder} WHERE user_id = {placeholder}
        ''', (user1_id, user2_id))
        ...
        cursor.execute(f'''
            INSERT INTO chat_sessions (user1_id, user2_id) VALUES ({placeholder}, {placeholder})
        ''', (user1_id, user2_id))
        cursor.close()
    ...
    def end_chat_session(self, user_id):
        ...
        # Clear chat partners
        cursor.execute(f'UPDATE users SET chat_partner = NULL WHERE user_id = {placeholder}', (user_id,))
        if partner_id:
            cursor.execute(f'UPDATE users SET chat_partner = NULL WHERE user_id = {placeholder}', (partner_id,))
            
            # Update chat session as ended
            if self.is_sqlite:
                cursor.execute(f'''
                    UPDATE chat_sessions SET ended_at = CURRENT_TIMESTAMP, is_active = 0 
                    WHERE (user1_id = {placeholder} AND user2_id = {placeholder}) OR (user1_id = {placeholder} AND user2_id = {placeholder})
                    AND is_active = 1
                ''', (user_id, partner_id, partner_id, user_id))
            else:
                cursor.execute(f'''
                    UPDATE chat_sessions SET ended_at = CURRENT_TIMESTAMP, is_active = FALSE 
                    WHERE (user1_id = {placeholder} AND user2_id = {placeholder}) OR (user1_id = {placeholder} AND user2_id = {placeholder})
                    AND is_active = TRUE
                ''', (user_id, partner_id, partner_id, user_id))
    ...
    def log_message(self, sender_id, receiver_id, message_type, message_content):
        if not self._ensure_connection():
            return
        cursor = self.connection.cursor()
        placeholder = self._placeholder()
    ...
    def get_all_users(self):
        ...
        cursor = self.connection.cursor()
        if self.is_sqlite:
            cursor.execute('SELECT user_id FROM users WHERE agreed_terms = 1 AND is_blocked = 0')
        else:
            cursor.execute('SELECT user_id FROM users WHERE agreed_terms = TRUE AND is_blocked = FALSE')
