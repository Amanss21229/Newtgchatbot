...
Enjoy chatting anonymously!
    ...
    async def chat(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Check VIP status and partner filter
        gender_filter = None
        if user_data['is_vip'] and user_data['vip_until'] and datetime.fromisoformat(str(user_data['vip_until'])) > datetime.now():
            gender_filter = user_data['partner_filter']
        
        # Find partner
        partner_id = db.find_chat_partner(user_id, gender_filter)
        
        if not partner_id:
            await update.message.reply_text("⏳ Looking for a chat partner... Please try again in a moment.")
            return
